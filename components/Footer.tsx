...
interface FooterProps {
  onNavigate?: (view: 'home' | 'eligibility' | 'contact' | 'about') => void;
}

export const Footer: React.FC<FooterProps> = ({ onNavigate }) => {
  
  const handleNav = (e: React.MouseEvent, view: 'home' | 'eligibility' | 'contact' | 'about', sectionId?: string) => {
    if (onNavigate) {
        e.preventDefault();
        onNavigate(view);
        if (sectionId) {
        ...
        <div className="grid grid-cols-2 gap-12 w-full md:w-auto mt-8 md:mt-0">
          <div>
            ...
            <ul className="space-y-2 text-sm text-white/60">
              <li><button onClick={(e) => handleNav(e, 'home', 'hero')} className="hover:text-brand-orange transition-colors text-left w-full">Home</button></li>
              <li><button onClick={(e) => handleNav(e, 'about')} className="hover:text-brand-orange transition-colors text-left w-full">About</button></li>
              <li><button onClick={(e) => handleNav(e, 'eligibility')} className="hover:text-brand-orange transition-colors text-left w-full">Eligibility</button></li>
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold uppercase tracking-widest text-xs mb-4">Connect</h4>
             <ul className="space-y-2 text-sm text-white/60">
              ...
              <li>
                <button onClick={(e) => handleNav(e, 'contact')} className="hover:text-brand-orange transition-colors text-left w-full">
                  Contact
                </button>
              </li>
            </ul>
